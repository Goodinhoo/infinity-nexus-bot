...
const messages = [
    { mensagem: `broadcast &aO servidor reiniciará em 5 minutos! &bMotivo: %motivo%`, tempo: 2 * 60 * 1000 },
    { mensagem: `broadcast &aO servidor reiniciará em 3 minutos! &bMotivo: %motivo%`, tempo: 2 * 60 * 1000 },
    { mensagem: `broadcast &aO servidor reiniciará em 1 minuto, deslogue para evitar perdas!`, tempo: 30 * 1000 },
    { mensagem: `broadcast &aO servidor reiniciará em 30 segundos!`, tempo: 20 * 1000 },
    { mensagem: `broadcast &aO servidor reiniciará em 10 segundos!`, tempo: 5 * 1000 },
    { mensagem: `broadcast &aO servidor reiniciará em 5 segundos!`, tempo: 1 * 1000 },
    { mensagem: `broadcast &aO servidor reiniciará em 4 segundos!`, tempo: 1 * 1000 },
    { mensagem: `broadcast &aO servidor reiniciará em 3 segundos!`, tempo: 1 * 1000 },
    { mensagem: `broadcast &aO servidor reiniciará em 2 segundos!`, tempo: 1 * 1000 },
    { mensagem: `broadcast &aO servidor reiniciará em 1 segundo!`, tempo: 1 * 1000 },
    { mensagem: `broadcast &aO servidor será reiniciado!`, tempo: 2 * 1000 },
    { mensagem: `kick all &7[&dInfinity Nexus&7] \\n\\n&aO servidor está reiniciando! \\n\\n&aRazão: %motivo%\\n\\n&6Entre no nosso Discord para saber mais: &dhttps://discord.gg/3gDrregs5n`, tempo: 5 * 1000 },
    { mensagem: `save-all`, tempo: 5 * 1000 },
    { mensagem: 'restart', tempo: 5 * 1000 }, //Função
];

module.exports = {
    data: new SlashCommandBuilder()
        .setName('reiniciar')
        .setDescription('Reinicia o servidor.')
        .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
        .addStringOption(option =>
            option.setName('option')
                .setDescription('Start or Stop reload system')
                .setRequired(true)
                .addChoices(
    ...
    execute: async (client, interaction) => {
        ...
        try {
            ...
            if (option == 'start') {
                if (timer) return interaction.reply({ content: `O servidor já está em processo de reinicialização!\nfoi por: <@${player}>`, ephemeral: true });
                interaction.reply({ content: `O servidor foi colocado para reiniciar!\n**motivo:** ${motivo}`, ephemeral: true })
                ...
                consoleServer('broadcast &aA reinicialização foi cancelado!');
                interaction.reply({ content: `O Sistema de reload, foi desligado!`, ephemeral: true })
                stopReload();
            }
        } catch (e) {
            if (client.errorLog) {
